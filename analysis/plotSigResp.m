function [h_plt,freq_in_GHz,amp_in_dB] = plotSigResp(signal,nfft,fs,lineType)
% Description:
%     plot signal response
%
% EXAMPLE:
%     [h_plt,freq_in_GHz,amp_in_dB] = plotSigResp(signal,nfft,fs,lineType)
%
% INPUT:
%     signal             - input signal
...
% V1.0       20171120    H.B. Zhang    replace psd with pwelch
...
% [Pxx, Pxxc, freq]       = psd(signal,nfft,fs);

% Welch£ºÆ×ÃÜ¶È·½·¨
window                  = hanning(nfft); % same as psd function window
noverlap                = 0;
range                   = 'twoside';       % [0, fs/2]
[ampTmp,freq]           = pwelch(Ei,window,noverlap,nfft,fs,range);
% calc power spectra
Pxx                     = ampTmp*fs/2;
Pxx(1)                  = Pxx(1)*2;
Pxx(length(Pxx))        = Pxx(length(Pxx))*2;

freq_in_GHz             = freq(:)*1E-9;
ref_freq_min            = ref_freq_min*1E-9;
ref_freq_idx            = find(freq_in_GHz > ref_freq_min,1,'first');
amp_in_dB               = 10*log10(Pxx(:)/Pxx(ref_freq_idx));

h_plt                   = plot(freq_in_GHz, amp_in_dB, lineType, 'LineWidth', lineSize);
